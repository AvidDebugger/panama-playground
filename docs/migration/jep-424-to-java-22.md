# JEP 424 to java 22 Migration Guide

This guide maps JEP 424 (Foreign Function & Memory API, Preview) concepts to the final Java 22 API.

## Overview

The FFM API was introduced as a preview in JEP 424 (JDK 19) and finalized in JEP 454 (JDK 22). java 22 uses the final API without preview flags.
...
- **Java 22**: `Arena` does the same. Use `try-with-resources` for confined/shared arenas.

## Symbol Lookup

- **JEP 424**: `SymbolLookup.libraryLookup(Path, ClassLoader)` or similar.
- **Java 22**: `SymbolLookup.libraryLookup(String name, Arena arena)` or `libraryLookup(Path path, Arena arena)`. The library is unloaded when the arena is closed. Use `Arena.global()` for long-lived lookups.

## Downcalls

- **JEP 424**: `CLinker.downcallHandle(address, ...)`.
- **Java 22**: `Linker.downcallHandle(segment, FunctionDescriptor)` or `(segment, FunctionDescriptor, options)`. For struct-by-value returns, the allocator is inserted as the first parameter.

## Upcalls

- **JEP 424**: `CLinker.upcallStub(handle, descriptor, session)`.
- **Java 22**: `Linker.upcallStub(handle, descriptor, arena)`.
...
Java 22 FFM is final. Do **not** use `--enable-preview` for FFM.
